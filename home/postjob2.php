<?php
	...
	if(isset($_POST['generate'])){
		...
		if($return){
				...
				$sql = "INSERT INTO job_posting(name, date, description) VALUES (?,?,?);";
				$stmtinsert = $db->prepare($sql);
				$result = $stmtinsert->execute([$name, $lastname, $date, $description]);
				if($result){
					if ($stmtinsert->rowCount()>0) {
							echo "<script>
								Swal.fire({
									title: 'Job Post Successful',
									text: 'You have succesfully posted a job.',
									icon: 'success'
								})
								var r = 'profile.php';
				...
				echo "<script>
								Swal.fire({
									...
									text: 'Job or Project already exists!',
									icon: 'error'
								})
							</script>";
			}
...
<!-- <label for="subCatRadio_10" class="columnSelection__label">Industry Specific Expertise</label> -->
	   ...
	   <div style="text-align: left; margin-left: 5px;">
	    ...
	    <input type="submit" value="Submit" name="generate" id="postjob">
  </form>
</div>
